  ...
  return (
    <section ref={sectionRef} id="hero" style={{ minHeight: '100vh', display: 'grid', gridTemplateColumns: '1fr 1fr', overflow: 'hidden', position: 'relative' }}>
      ...
      >
        ...
        {/* Heading — animate on mount, clipped line-by-line reveal */}
        <h1
          className="font-cormorant font-light leading-[0.92] tracking-[-0.025em] text-cream"
        ...
        >
          <span style={{ display: 'block', overflow: 'hidden' }}>
            <motion.span
              ...
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              transition={{ delay: 0.48, duration: 0.95, ease: E }}
            >
              Mushaf
            </motion.span>
          </span>
          <span style={{ display: 'block', overflow: 'hidden' }}>
            <motion.span
              ...
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              transition={{ delay: 0.64, duration: 0.95, ease: E }}
            >
              <em className="italic text-gold">Fatma</em>
            </motion.span>
          </span>
        </h1>

        <motion.div variants={FADE} initial="hidden" animate="show" custom={0.88}
        ...
        <motion.p variants={FADE} initial="hidden" animate="show" custom={1.0}
        ...
        <motion.div variants={FADE} initial="hidden" animate="show" custom={1.15}
          ...
          <MagneticButton>
            <a href="#portfolio"
              ...
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow = ''; (e.currentTarget as HTMLElement).style.background = '#C8A86B' }}>
              View Work
            </a>
          </MagneticButton>
          <MagneticButton>
        ...
        <motion.div variants={FADE} initial="hidden" animate="show" custom={1.3} className="flex gap-12 mt-16">
      ...
      <div className="relative overflow-hidden"
        ...
        {/* Aurora fluid gradient */}
        <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none', zIndex: 0 }}>
          <div style={{
            position: 'absolute', top: '50%', left: '50%',
            width: '160%', height: '160%',
            background: 'conic-gradient(from 0deg at 50% 50%, rgba(200,168,107,0.07) 0deg, rgba(50,30,100,0.09) 90deg, rgba(10,24,48,0.07) 180deg, rgba(120,50,50,0.07) 270deg, rgba(200,168,107,0.07) 360deg)',
            animation: 'aurora-rotate 24s linear infinite',
            filter: 'blur(72px)',
            borderRadius: '50%',
          }} />
          <div style={{
            position: 'absolute', inset: 0,
            background: 'radial-gradient(ellipse 65% 55% at 30% 70%, rgba(200,168,107,0.08) 0%, transparent 60%), radial-gradient(ellipse 45% 65% at 72% 28%, rgba(35,20,80,0.11) 0%, transparent 55%)',
            animation: 'aurora-pulse 7s ease-in-out infinite',
          }} />
        </div>

        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 70% 70% at 50% 50%, rgba(200,168,107,0.04) 0%, transparent 70%)', pointerEvents: 'none', zIndex: 1 }} />
        ...
        {PARTICLES.map((p, i) => (
          <motion.div key={i}
            style={{ position: 'absolute', left: p.x, top: p.y, width: p.s, height: p.s, borderRadius: '50%', background: '#C8A86B', boxShadow: `0 0 ${p.s * 5}px rgba(200,168,107,0.65)`, pointerEvents: 'none', zIndex: 3 }}
