          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 28, marginTop: 56 }}>
            {[
              ...
              <motion.div
                ...
                whileInView={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.7, delay: 0.28 + i * 0.1, ease: [0.16, 1, 0.3, 1] }}
                viewport={VP}
        ...
        >
          ...
          <div style={{ marginBottom: 32 }}>
            ...
            <select defaultValue="">
              ...
              {['Under AED 1,000', 'AED 1,000 – 5,000', 'AED 5,000 – 15,000', 'AED 15,000+'].map(s => <option key={s}>{s}</option>)}
            </select>
          </div>
          <div style={{ marginBottom: 32 }}>
            ...
            <textarea placeholder="Tell me about your project, timeline, and what you need..." />
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <button type="submit" data-hover
              style={{ display: 'inline-flex', alignItems: 'center', gap: 14, padding: '16px 48px', background: sent ? '#C8A86B' : 'transparent', border: '1px solid #8B6B3A', color: sent ? '#0B0A08' : '#C8A86B', fontFamily: '"DM Sans",sans-serif', fontSize: 11, letterSpacing: '0.16em', textTransform: 'uppercase', borderRadius: 2, transition: 'all .4s' }}
              onMouseEnter={e => { if (!sent) { const b = e.currentTarget; b.style.background = '#C8A86B'; b.style.color = '#0B0A08'; b.style.borderColor = '#C8A86B' } }}
              onMouseLeave={e => { if (!sent) { const b = e.currentTarget; b.style.background = 'transparent'; b.style.color = '#C8A86B'; b.style.borderColor = '#8B6B3A' } }}>
              {sent ? 'Message Sent ✓' : <><span>Send Message</span><span style={{ fontSize: 18 }}>→</span></>}
            </button>
          </div>
        </motion.form>
      </div>
